  ...
  return (
    ...
    <div className=' mt-2 pl-2 w-screen relative'>
     ...
     <>
       ...
       <div  className={`flex gap-2 justify-center absolute -z-1  border-2 rounded-xl py-4 px-6 font-semibold shadow-lg shadow-slate-300  hover:-translate-y-0.5 bg-[#000080] text-white ml-9 right-8 mb-2 ${isPopupVisible?"hidden":"block"} `}><Icon icon="gala:add" width="22" /> <div  >Create Assignment</div></div>
       </Popover.Button>
       <Popover.Panel  className="absolute right-8 -mt-8 z-10  w-[calc(100vw-6rem)] shadow-sm rounded-md shadow-black bg-[#dcdcdc] h-[calc(100vh-5rem)]" style={{
...
<div className='  px-2 py-3  text-sm'>
      ...
      <label className=' text-gray-700  font-semibold mb-2'>
        Subjects: </label>
        <select className='ml-16 text-lg shadow  border rounded w-[70%] py-4 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' value={newAssignment.select} onChange={(e) => setNewAssignment({ ...newAssignment, select: e.target.value })}>
        <option></option>
        <option>Math</option>
      <option>c++</option>
     ...
     <div className='flex flex-wrap gap-8 mt-20 ml-8  '>
      {
        data.map((item)=><div className='bg-[#FAFAFA]  -z-1 cursor-pointer hover:scale-105 w-[22%] rounded-lg  p-4 shadow-gray-400 shadow-md' key={item.id} onClick={() => handleCardClick(item)}>
    
            <div className='relative text-[18px] text-[#435585] font-bold '>{item.select}</div> 
...
<ul className="list-none p-0  ">
  {submittedStudents.map((student, index) => (
    <li
      key={index}
      className="mb-5 p-5 bg-gray-200 rounded-md"
    >
      {student}
    </li>
  ))}
